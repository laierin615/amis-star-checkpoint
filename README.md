...
- 問卷關卡：按「完成問卷關卡」後直接算完成，問卷另開分頁，做完回來繼續闖關。
...
3. 完成報名後，系統自動產生參加者代碼與 QR code，並核發第 1 關「完成報名」菱形。
4. 產生第 3 到第 8 關的關主給點流程；參加者代碼由 QR 自動帶入，關主只輸入給點密碼。
5. 印出或發送參加者 QR 卡，關主手機先測試掃碼與核發菱形。

### 2. 闖關中

1. 完成報名後，第 1 關「完成報名」菱形亮起。
2. 第 2 關問卷按下後直接完成，問卷另開分頁。
3. 參加者完成第 3 到第 8 關任務後，出示自己的專屬 QR 或參加者代碼。
4. 關主用自己的手機開啟「關主給點」頁。
5. 關主掃參加者 QR 後，系統自動帶入參加者代碼。
6. 關主確認任務通過後，只輸入給點密碼並按「核發菱形」。
7. 系統檢查：給點密碼是否正確、參加者是否存在、該關是否已經給過點。
8. 成功後該菱形亮起，八關集滿後組成阿美族八角星圖騰。
...
| 欄位 | 說明 |
| --- | --- |
| `station_id` | 關卡代碼，例如 `s1` 完成報名、`s2` 問卷、`s3` 到 `s8` 為關主核發關卡 |
| `station_name` | 關卡名稱 |
| `host_name` | 關主姓名 |
