...
- 第一個菱形：完成報名後預設亮起，不需要關主核發。
- 問卷關卡：按「完成問卷關卡」後直接算完成，問卷另開分頁，做完回來繼續闖關。
- 給點密碼：第 2 到第 7 關由關主輸入指定密碼核發菱形。
...
注意：目前靜態版使用瀏覽器本機 `localStorage`，適合用同一台報名裝置從 `001` 依序發號；若要多台手機同時報名且全域保證不重複，需要接 Google Sheet、Firebase 或其他後端。

## 推薦流程

### 1. 活動前

1. 建立七個核發關卡資料：關卡名稱、關主姓名、關主手機、給點密碼、是否啟用。
2. 活動開始前或現場入口先收集姓名與身分別，身分別固定為「學生、來賓、家長、師長」。
3. 完成報名後，系統自動產生該參加者獨一無二的 QR code；QR 內容是帶有參加代碼的報名集點卡網址。
4. 產生第 2 到第 7 關的關主給點流程；參加者代碼由 QR 自動帶入，關主只輸入給點密碼。
5. 印出或發送參加者 QR 卡，關主手機先測試掃碼與核發菱形。

### 2. 闖關中

1. 完成報名後，起點菱形預設亮起。
2. 第 1 關問卷按下後直接完成，問卷另開分頁。
3. 參加者完成第 2 到第 7 關任務後，出示自己的專屬 QR 或參加者代碼。
4. 關主用自己的手機開啟「關主給點」頁。
5. 關主掃參加者 QR 後，系統自動帶入參加者代碼。
6. 關主確認任務通過後，只輸入給點密碼並按「核發菱形」。
7. 系統檢查：給點密碼是否正確、參加者是否存在、該關是否已經給過點。
8. 成功後該菱形亮起，起點菱形加上七關菱形集滿後組成阿美族八角星圖騰。
...
| 欄位 | 說明 |
| --- | --- |
| `station_id` | 關卡代碼，例如 `s1` 為起點菱形，`s2` 到 `s8` 為七個核發關卡 |
| `station_name` | 關卡名稱 |
| `host_name` | 關主姓名 |
...
`stamps` 需要設定唯一限制：同一個 `participant_id + station_id` 只能存在一筆，這樣可以阻擋同一關重複核發菱形。抽獎名單由 `stamps` 中完成起點菱形加七個核發關卡的參加者自動產生。
